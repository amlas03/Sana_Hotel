...
public class GeneralSearch extends JPanel implements ActionListener{
	...
	public GeneralSearch(attributsSearch a) {
		 b=new BackgroundPanel("gradient.jpg");
		...
		JPanel p=new JPanel();
		p.add(b1); p.add(b2); p.setOpaque(false); p.setPreferredSize(new Dimension(200,-250));
		
		 p1=new JPanel(); p1.setLayout(new BoxLayout(p1, BoxLayout.Y_AXIS));
		id=new JLabel("Veuillez entrer l'élément correspondant: ");
		id.setBorder(new EmptyBorder(30,0,0,0));
		p.add(id);
		p1.add(p);
		la=new JLabel("SALMA");
		//p1.add(la); 
		...
		p2.add(l); p2.add(search); p2.setOpaque(false);
		p1.add(p2);
		...
		b.setLayout(new BoxLayout(b, BoxLayout.Y_AXIS));
		 ...
		 la.setBorder(new EmptyBorder(-800,0,0,0));JPanel laa=new JPanel();
		p1.add(la);
		b.add(p1);
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource()==search) {
			...
			try {
				la=new JLabel((rd.getReservationsByClientId(i)).toString());
